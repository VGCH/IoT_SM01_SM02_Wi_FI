/* Модуль правления освещением SM-01
 *  © CYBEREX Tech, 2026
 * 
 */
...
#define on_off_pin    12                    // Пин управления питанием
#define on_off_ind    4                     // Индикатор включения 
#define button_contr  13                    // Кнопка управления
#define external_switch  14                 // Пин для подключения внешнего управления
...
const char *myHostname  = "Cyberex_SmartSw01";            // Имя создаваемого хоста Cyberex_SmartSw.local 
const char *softAP_ssid = "CYBEREX-SmartSw01";            // Имя создаваемой точки доступа Wi-Fi

String version_code = "RS-1.0.01.26";

float auto_on_dist;
//timers
uint32_t low_t, med_t, prevMills, lastMsg, impOnDelay, reboot_t, lastConnectTry, lastButton, lastSwitch;
...
void setup() {
    ...
    pinMode(external_switch, INPUT);                                // Режим пина внешнего выключателя
   ...
   if(String(settings.passwd) == ""){   
     ...
     WiFi.hostname("Smart Switch 01 [CYBEREX TECH]");                     // Указываеем имя хоста, который будет отображаться в Wi-Fi роутере, при подключении устройства
